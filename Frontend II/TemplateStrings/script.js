window.onload = () => {
    ...
    boton.addEventListener('click',(e) => {
        e.preventDefault();
        document.querySelector('body').classList.toggle('bodyModoOscuro');
    });   
    ...
    const noticias = [
        {
            ...
            titulo: "El tigre",
            contenido: `El tigre (Panthera tigris) es una de las especies​ de la subfamilia de los panterinos 
    (familia Felidae)   pertenecientes al género Panthera. Se encuentra solamente en el continente asiático; es un predador carnívoro y
    es
    ...
    for (let i = 0; i < noticias.length; i++) {
        crearNoticia(noticias[i].urlImagen,noticias[i].titulo,noticias[i].contenido);
    };
